...
public class SignerWorker extends SwingWorker<Void, Void> {
    ...
    protected Void doInBackground() {
        try {
            ...
            synchronized (PKCS11_LOCK) {
                this.document.sign(card);
            }
            if (this.document.getIsremote()){
                this.gui.signDone(this.document);
            }
         } catch (Exception e) {
