...
public class FirmadorASiC extends CRSigner implements DocumentSigner {
    ...
    public DSSDocument sign(DSSDocument toSignDocument, CardSignInfo card, Settings settings) {
        ...
        try {
            ...
            signedDocument = service.signDocument(detacheddocs, parameters, signatureValue);
	...
	private RemoteSignatureParameters getRemoteSignatureParameters(Document toSignDocument, CardSignInfo card) {
		...
		SignatureLevel xadeslevel = docSettings.getXAdESLevel();
		...
		remoteParameters.setSignatureLevel(xadeslevel);
		remoteParameters.setSignaturePackaging(SignaturePackaging.ENVELOPED);
		remoteParameters.setManifestSignature(true);
		// setManifestSignature
		remoteParameters.setAsicContainerType(ASiCContainerType.ASiC_E);
		return remoteParameters;
	}

	@Override
	public FirmadorRemoteDocument getTobeSignedRemote(Document toSignDocument, CardSignInfo card) {
		ASiCWithXAdESService service = new ASiCWithXAdESService(this.getCertificateVerifier(card.getCertificate()));
		OnlineTSPSource onlineTSPSource = new OnlineTSPSource(TSA_URL);
		service.setTspSource(onlineTSPSource);
		...
		remoteservice.setAsicWithXAdESService(service);
