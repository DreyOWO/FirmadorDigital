...
public class PKCS11Manager implements CardManagerInterface {
    ...
    public List<X509Certificate> searchCertificates() throws Throwable {
        ...
        for (long slotID : pkcs11.C_GetSlotList(tokenPresent)) {
            ...
            if ((slotInfo.flags & CKF_TOKEN_PRESENT) != 0) { // Not required if tokenPresent = true, condition could be
                                                             // removed if true, it's just for testing empty slot
                                                             // enumeration
                try { // FIXME: slotID may be reused after switching card! try CK_SESSION_INFO
                      // sessionInfo = pkcs11.C_GetSessionInfo(hSession); and catch PCKCS11Exception
                      // meaning invalid session instead!
                    CK_TOKEN_INFO tokenInfo = pkcs11.C_GetTokenInfo(slotID);
                    LOG.info("Token: " + new String(tokenInfo.label).trim() + " ("
                            + new String(tokenInfo.serialNumber).trim() + ")");
                    CK_ATTRIBUTE[] pTemplate = { new CK_ATTRIBUTE(CKA_CLASS, CKO_CERTIFICATE) };
                    long ulMaxObjectCount = 32;
                    long hSession = pkcs11.C_OpenSession(slotID, CKF_SERIAL_SESSION, null, null); // FIXME: verify slot
                                                                                                  // session just after
                                                                                                  // getting PIN but
                                                                                                  // just before login
                    ...
                    for (long object : phObject) {
                        CK_ATTRIBUTE[] pTemplate2 = { new CK_ATTRIBUTE(CKA_VALUE), new CK_ATTRIBUTE(CKA_LABEL) }; // if
                                                                                                                  // you
                                                                                                                  // add
                                                                                                                  // more
                                                                                                                  // attributes,
                        ...
                        for (int i = 0; i < pTemplate2.length; i = i + 2) { // iterator jump value to read just
                                                                            // certificates at pTemplate[0],
                                                                            // pTemplate[2]... FIXME: better use pValue
                                                                            // filtering
                            keyIdentifier = pTemplate2[i + 1].getCharArray();
                            X509Certificate certificate = (X509Certificate) CertificateFactory.getInstance("X.509")
                                    .generateCertificate(new ByteArrayInputStream((byte[]) pTemplate2[i].pValue));
                            certlist.add(certificate);
                            slot_by_cert.put(certificate.getSerialNumber(), slotID);
                            token_by_cert.put(certificate.getSerialNumber(), keyIdentifier);
                            LOG.debug("Public/Private key pair identifier: " + new String(keyIdentifier)); // After logging in with
                                                                                               // PIN, find the matching
                                                                                               // private key pValue.
                                                                                               // NOTE: Old certificates
                                                                                               // didn't use
                                                                                               // "LlaveDeFirma"
                                                                                               // id/label.
                            // FIXME: Don't assume there's a single valid certificate per token (Persona
                            // Jurídica keystores might contain more than 1 usable certificate per token as
                            // they are handmade)
                        }
                    }
                ...
                LOG.info("No token present in this slot"); // Condition could be removed
    ...
    public void logoutAndCloseAllSessions(long slotID) {
        try {
            PKCS11 pkcs11 = initialize();
            long hSession = pkcs11.C_OpenSession(slotID, CKF_SERIAL_SESSION, null, null);
            try {
                try {
                    pkcs11.C_Logout(hSession);
                } catch (PKCS11Exception e) {
                    LOG.debug("C_Logout ignorado (posiblemente no logueado): {}", e.getMessage());
                }
            } finally {
                pkcs11.C_CloseSession(hSession);
            }
        } catch (Throwable t) {
            LOG.warn("No se pudo forzar logout en slot {}: {}", slotID, t.toString());
        }

        this.keystore = null;
        this.lastSlotID = null;
        this.cachedListCertificates = null;
        try {
    ...
    public void login(long slotID, PasswordProtection pin) throws Throwable {
        PKCS11 pkcs11 = initialize();
        long hSession = pkcs11.C_OpenSession(
                slotID,
        ...
        try {
            pkcs11.C_Login(hSession, CKU_USER, pinChars);
            LOG.info("Login exitoso en slot {}", slotID);
        } catch (PKCS11Exception e) {
            if (e.getMessage() != null && e.getMessage().contains("CKR_USER_ALREADY_LOGGED_IN")) {
                LOG.debug("Usuario ya estaba logueado en slot {}", slotID);
            } else {
                throw e;
            }
        } finally {
            Arrays.fill(pinChars, '\0'); // limpiar por seguridad
        }
    }
