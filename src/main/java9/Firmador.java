...
public class Firmador {
    ...
    public static void main(String[] args) throws Throwable {
    	if (System.getProperty("os.name").startsWith("Mac")) {
            doMain(args); // macOS now uses its own bundled JRE launched from app bundle providing the --add-exports argument
            return;
        }
        for (String s : args) {
            if (s.equals("run")) {
    ...
    public static void doMain(String[] args) {
        ...
        gui.setSmartCardDetector(detector);
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				detector.start();
				try {
					detector.readSaveListSmartCard();
				} catch (Throwable e) {
